...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      // Check if user exists - use exec() to properly resolve the promise
      const userExists = await User.findOne({ email: req.body.email }).lean().exec();
      if (userExists) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      // Check for user - use exec() to properly resolve the promise
